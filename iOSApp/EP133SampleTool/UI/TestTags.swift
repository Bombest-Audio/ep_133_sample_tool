...
/// SwiftUI port of `AndroidApp/.../ui/TestTags.kt`.
///
/// Accessibility identifiers for UI tests — same string values as the Android semantics test tags
/// so the iOS UI tests mirror the Android robots. Naming: {screen}_{element} snake_case; indexed
/// or named elements use the function helpers. Tags are only added where text selectors are
/// ambiguous (nav label vs header title), structural (containers, dialogs), or dynamic (pads,
/// slots, import rows) — user-visible labels stay text assertions in tests.
enum TestTags {
    // App shell
